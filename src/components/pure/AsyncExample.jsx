...
const AsyncExample = () => {
    ...
    async function generatePromeseNumber(){
    ...
    function showStorage(){
        saveSessionStorage('name', 'Nerea')
            .then((response) => {
                let value = response;
                alert(`Dave name: ${value}`);
            }).catch((error) => {
    ...
    async function obtainMessage(){
        ...
        let message = await promese;

        await alert(`Message received: ${message}`);
    }

    const returnError = async() =>{
        await Promise.reject(new Error('Oooop'))
    }
